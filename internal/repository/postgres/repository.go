...
func (r *Repository) GetJob(ctx context.Context, jobID model.ID) (model.Job, error) {
	query := `
SELECT id, job_type, status, COALESCE(source_platform::text, ''), payload, result,
       scheduled_at, started_at, finished_at, retry_count, COALESCE(error_message, ''), created_at, updated_at
FROM jobs
WHERE id = $1`
	...
	var payload []byte
	var result []byte
	if err := r.db.QueryRowContext(ctx, query, jobID).Scan(
		&job.ID,
		&job.JobType,
		&job.Status,
		&sourcePlatform,
	...
	job.SourcePlatform = model.SourcePlatform(sourcePlatform)
	if err := unmarshalJSON(payload, &job.Payload); err != nil {
		return model.Job{}, err
	}
	if err := unmarshalJSON(result, &job.Result); err != nil {
		return model.Job{}, err
	}
